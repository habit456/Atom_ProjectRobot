...
//Picks up whatever mail is closest and then delivers whatever address is closest.
function proximityRobot({place, parcels}, route) {
  if (route.length == 0) {
    if (!parcels.every(p => p.place == place)) {
      let placeProps = parcels.filter(p => p.place != place).map(p => p.place);
      route = shortestRoute(placeProps, place);
    } else {
      let addressProps = parcels.map(p => p.address);
      route = shortestRoute(addressProps, place);
    }
  }
  return {direction: route[0], memory: route.slice(1)};
}
...
/*
  Helper function for proximityRobot. Finds the shortest route from where the
  robot is to a given array of locations.
*/
function shortestRoute(props, place) {
  let work = [], shortest = [];
  props.map(p => {
    work.push(findRoute(roadGraph, place, p));
  });
  work.map(r => {
...
/*
  Compares 2 robots by averaging the amount of turns it takes for each to
  complete a given amount of random town cycles.
*/
function compareRobots(robot1, memory1, robot2, memory2, numtasks = 100) {
  let avg1 = 0, avg2 = 0, myVillage;
  for (let i = 0; i < numtasks; i++) {
    myVillage = VillageState.random();
    avg1 += runRobot(myVillage, robot1, memory1);
    avg2 += runRobot(myVillage, robot2, memory2);
  }
  avg1 /= numtasks;
  avg2 /= numtasks;
  console.log(`Robot 1 averaged ${avg1} turns per task.
Robot 2 averaged ${avg2} turns per task`);
  if (avg1 < avg2) {
    console.log(`Robot 1 is ${(avg2 - avg1).toFixed(4)} turns faster than Robot 2.`);
  } else {
    console.log(`Robot 2 is ${(avg1 - avg2).toFixed(4)} turns faster than Robot 1.`);
  }
}



compareRobots(proximityRobot, [], routeRobot, mailRoute, 1000);
